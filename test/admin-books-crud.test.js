...
test('AdminService CRUD operations for Books entity', async () => {
  const newBook = { title: 'Admin Test Book', stock: 3, price: 10.99 };
  ...
  // Read
  res = await GET(`/odata/v4/admin/Books(${ID})`);
  assert.equal(res.status, 200);
  assert.equal(res.data.title, newBook.title);

  // Update
  res = await PATCH(`/odata/v4/admin/Books(${ID})`, { stock: 5 });
  assert.equal(res.status, 200);
  res = await GET(`/odata/v4/admin/Books(${ID})`);
  assert.equal(res.data.stock, 5);

  // Delete
  res = await DELETE(`/odata/v4/admin/Books(${ID})`);
  assert.equal(res.status, 204);

  // Ensure deletion
  res = await GET(`/odata/v4/admin/Books(${ID})`).catch(err => err.response);
